...
public:
    ...
    bool Recover(const uint256 &hash, const unsigned char *p64, int rec)
    {
        ...
        BIGNUM* rbn = BN_dup(ECDSA_SIG_get0_r(sig));
        BIGNUM* sbn = BN_dup(ECDSA_SIG_get0_s(sig));
        BN_bin2bn(&p64[0],  32, rbn);
        BN_bin2bn(&p64[32], 32, sbn);
        if (ECDSA_SIG_set0(sig, rbn, sbn) != 1)
            return false;
