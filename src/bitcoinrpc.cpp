...
{
    return GetBoolArg("-testnet", false) ? 13342 : 3342;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Boundless server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Boundless server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: boundless-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
public:
    ...
    {
        ip::tcp::resolver resolver(static_cast<asio::io_context&>(stream.get_executor().context()));
...
{
    // Accept connection
  AcceptedConnectionImpl<Protocol>* conn = new AcceptedConnectionImpl<Protocol>(static_cast<asio::io_context&>(acceptor->get_executor().context()), context, fUseSSL);
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use boundlessd";
...
void StopRPCThreads()
{
    ...
    delete rpc_io_context; rpc_io_context = nullptr;
