...
#if defined(NDEBUG)
# error "Boundless cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x29aa0fb842da2afb38951b48a74ae9637f0c9ec4affe8625d059501ffc816d96");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Boundless: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Boundless Signed Message:\n";
...
{
    // Boundless: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Boundless: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Boundless: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Boundless: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8295321627ed3063e872aadc24995090d2085ecb43794ec3933e0bcd332a6db4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN.com 10/6/2023 Shares in this UK Bank just plunged over 30%.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b1f7388c94f4861c52111e314430528e43e2381639270946765a980230708353338b971f41f7a9d4c692bdfff13e40b8052975e22dc8a40745548cdd27ce3234") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nNonce   = 2087496204;

        if (fTestNet)
        {
            block.nTime    = 1696587228;
            block.nNonce   = 386320780;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd154f1842bc73f25d387d5a20d0da32e6d8029316ea78e79a5b6cbe919867afa"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc1, 0xb2, 0xd3 }; // Boundless: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BoundlessMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BoundlessMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BoundlessMiner(CWallet *pwallet)
{
    printf("BoundlessMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("boundless-miner");
    ...
    try { loop {
        ...
        printf("Running BoundlessMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BoundlessMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BoundlessMiner, pwallet));
}
