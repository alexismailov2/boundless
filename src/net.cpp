...
#  endif
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Boundless " + FormatFullVersion();
...
//{"167.172.129.199","167.172.129.199"},
 {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Boundless is probably already running."), addrBind.ToString().c_str());
        else
